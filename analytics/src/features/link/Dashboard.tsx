...
export default function Dashboard({ title, onOpenCreate }: Props) {
  // URL ?slug=... 로 통계 대상 링크 지정 (없으면 기본값)
  const slug = React.useMemo(
    () => new URLSearchParams(window.location.search).get("slug") ?? "V8aNfV",
    []
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 to-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* 3번 카드: analytics의 4개 그래프를 통째로 임베드 */}
          <BoardCard title="통계">
            <div className="rounded-xl overflow-auto bg-white max-h-[78vh]">
              {/* embed 모드 → 헤더/여백 제거, 그래프만 표시 */}
              <AnalyticsDashboard embed slug={slug} />
            </div>
          </BoardCard>
